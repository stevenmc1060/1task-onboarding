...
const ChatInterview = ({ userData, onComplete }) => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Check if interview is complete (look for very specific completion indicators)
        const responseText = data.response.toLowerCase();
        if ((responseText.includes('dashboard is ready') && responseText.includes('setup complete')) ||
            (responseText.includes('final summary') && responseText.includes('areas')) ||
            (responseText.includes('all set') && responseText.includes('dashboard')) ||
            responseText.includes('onboarding is now complete') ||
            responseText.includes('your personalized setup is complete') ||
            // More strict fallback - need 8+ exchanges AND specific completion words
            (newHistory.length >= 8 && (
          ...
          console.log('Interview marked as complete. Response:', data.response);
  ...
  const parseInterviewResponses = (responses) => {
    ...
    // Extract focus areas (work, personal, health, etc.)
    const focusAreaKeywords = [
      'work', 'professional', 'career', 'job',
      'personal', 'growth', 'learning', 'development',
      'health', 'fitness', 'self-care', 'wellness',
      'family', 'relationships', 'social',
      'finances', 'financial', 'money',
      'community', 'volunteering', 'service',
      'hobbies', 'creativity', 'art'
    ];

    focusAreaKeywords.forEach(keyword => {
      if (conversationText.includes(keyword)) {
        if (!structuredData.focus_areas.includes(keyword)) {
    ...
    if (responses.length > 2) {
      structuredData.bio = `Focused on ${structuredData.focus_areas.slice(0, 3).join(', ')} with ${structuredData.communication_style} communication style.`;
    }
